...
class AStarPlanner(object):
    ...
    # Modify it to implement A*. The changes should be minor. DONE.
    #
    def plan(self, start_state, dest_state):
        ...
        while Q:
            ...
            for ns in self.get_neighboring_states(s):
                ...
                # a heuristic for approximating the distance between a neighboring state and the destination state
                heuristic_distance = sqrt((ns.x - dest_state.x)**2 + (ns.y - dest_state.y)**2)
                transition_distance = sqrt((ns.x - s.x)**2 + (ns.y - s.y)**2)
                alternative_best_dist_ns = best_dist_found[s.x, s.y] + transition_distance

                # we additionaly consider an approximation for the distance from ns to the dest_state, before expanding nodes:
                if (ns not in Q) or (alternative_best_dist_ns < best_dist_found[ns.x, ns.y]):
                    best_dist_found[ns.x, ns.y] = alternative_best_dist_ns
                    Q[ns] = alternative_best_dist_ns + heuristic_distance
                    parents[ns] = s
...
if __name__ == "__main__":
    ...
    # in order to produce your images to submit. DONE.
    
    dest_state = State(300,300)                     # 0
    # dest_state = State(380,390)                     # 1
    # dest_state = State(300,200)                     # 2
