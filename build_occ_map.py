...
class HuskyMapper:
    ...
    def from_laser_to_map_coordinates(self, points_in_baselaser_frame):
        ...
        """
        Idea:
        - Since the following variables update upon every scan, load these into our list of numpy triplets:
        #self.baselaser_x_in_map
        #self.baselaser_y_in_map
        Go to global frame in 2D-Plane by subtracting origin
        Since we are converting to the 2D-Plane, we do not need to load the angle (third component)
        => O(n) where n == len(points_in_baselaser_frame)
        """

        points_in_map_frame = [np.array([self.baselaser_x_in_map-self.ogm.origin_x, self.baselaser_y_in_map-self.ogm.origin_y, float(0)]) for point in points_in_baselaser_frame] 
    ...
    def is_in_field_of_view(self, robot_row, robot_col, laser_theta, row, col):
        ...
        #
        """
        Idea: https://stackoverflow.com/questions/1167022/2d-geometry-how-to-check-if-a-point-is-inside-an-angle
        Implementing the algorithm found on this page. Calculate the angle of the direction using arctan(vector_cell_to_robot).
        Substract the origin from the cell of interest (normalized in 2 space).
        Calculate its angle (again via arctan of a normalized vector).
        Does this lie within angle boundaries? What about the distance of the initial vector - is this larger than the beam?
        """
        isWithinBounds = False

        normalized_cell = (row-self.ogm.origin_x, col-self.ogm.origin_y)
        robot = (robot_row, robot_col)

        robot_to_cell = (robot[0]-normalized_cell[0], robot[1]-normalized_cell[1])                                # vector
        metric_distance = self.ogm.euclidean(normalized_cell, robot) * self.ogm.meters_per_cell

        bounding_angle = atan2(*robot_to_cell)
        angle_of_interest = atan2(*normalized_cell)

        within_angle_limits = angle_of_interest < bounding_angle
        within_distance_limits = self.min_laser_range < metric_distance < self.max_laser_range
    ...
    def inverse_measurement_model(self, row, col, robot_row, robot_col, robot_theta_in_map, beam_ranges, beam_angles):
        ...
        # r should be in meters and diff_angle should be in [-pi, pi]. Useful things to know are same as above. DONE
        #
        # wall = (row, col)
        wall = (row-self.ogm.origin_x, col-self.ogm.origin_y)                   # Normalized (subtract origin)
        robot = (robot_row, robot_col)
        beam_vector = (wall[1]-robot[1], wall[0]-robot[0])     

        beam_metric_distance = self.ogm.euclidean(wall, robot) * self.ogm.meters_per_cell
        r = beam_metric_distance                                                # DONE

        diff_angle = 0.0
        difference_in_angle = atan2(*beam_vector) - robot_theta_in_map          # formula: angle of beam - angle of robot
        if (-pi <= difference_in_angle <= pi):
            diff_angle = difference_in_angle                                    # DONE
    ...
    def process_laser_scan(self, ranges_in_baselaser_frame):
        ...
        #       range sensed at the angle that corresponds to each beam. DONE.
        #  
        # Things to know:
        ...
        while (i < len(self.angles_in_baselaser_frame)):                        # len(self.angles_in_baselaser_frame)
            r = ranges_in_baselaser_frame[i]                                    #               == 
            angle = self.angles_in_baselaser_frame[i]                           # len(ranges_in_baselaser_frame)
            x = r*cos(angle)
            y = r *sin(angle)
            points_xyz_in_baselaser_frame.append(np.array([x, y, float(0)]))    # 2D-plane rep'n of where the lazer thinks the point is
            i += 1
...
if __name__ == '__main__':
    ...
    with open(r'C:\Users\kam\Desktop\curCourses\Fall 2020\COMP 417\A4\COMP417_Fall2020\A4\estimation_question\husky_data.yaml') as file:
