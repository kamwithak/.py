...
class RRTPlanner(object):
    ...
    def steer_towards(self, s_nearest, s_rand, max_steering_radius):
        """
        ...
        # and {0, ..., rows -1} respectively. DONE!
        x = 0
        y = 0
        
        distance_to_center = s_rand.euclidean_distance(s_nearest)       # compute euclidean distance to the center
        
        if (distance_to_center < max_steering_radius):                  # if less than radius
            x, y = int(s_rand.x), int(s_rand.y)
        else:
            distance_between_points = np.sqrt((s_rand.x - s_nearest.x) ** 2 + (s_rand.y - s_nearest.y) ** 2)
            distance_ratio = max_steering_radius / distance_between_points
            x = int(s_nearest.x + distance_ratio * (s_rand.x - s_nearest.x))
            y = int(s_nearest.y + distance_ratio * (s_rand.y - s_nearest.y))
    ...
    def path_is_obstacle_free(self, s_from, s_to):
        ...
        for i in range(max_checks):
            ...
            # away on the line from s_from to s_new is free or not. If not free return False. DONE!
            while (s_from.__eq__(s_to)):
                s_to = self.sample_state()
            distance_between_points = np.sqrt((s_to.x - s_from.x) ** 2 + (s_to.y - s_from.y) ** 2)      # similar to steer_towards()
            interpolated_state = float(i)/max_checks * s_from.euclidean_distance(s_to)
            distance_ratio = interpolated_state / distance_between_points                   
            x = int(s_from.x + distance_ratio * (s_to.x - s_from.x))                        # calculate new point, convert to int
            y = int(s_from.y + distance_ratio * (s_to.y - s_from.y))
            if (not self.state_is_free(State(x, y, None))):                                 # create a state and check if it is free
                return False
    ...
    def plan(self, start_state, dest_state, max_num_steps, max_steering_radius, dest_reached_radius, live_view=True):
        ...
        for step in range(max_num_steps):
            ...
            # in the output.
            s_rand = self.sample_state()                                                                                # DONE!
            s_nearest = self.find_closest_state(tree_nodes, s_rand)                                                     # DONE!
            s_new = self.steer_towards(s_nearest=s_nearest, s_rand=s_rand, max_steering_radius=max_steering_radius)     # DONE!
